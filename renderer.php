...
class core_course_renderer extends plugin_renderer_base {
    ...
    public function course_section_cm_list($course, $section, $sectionreturn = null, $displayoptions = array()) {
        ...
        //Sentencia para consulta a la base de datos de moodle 3.6

 $table = 'role_assignments'; //tabla de moodle en donde se encuentra el rol de los usuarios
$select = "userid = $USER->id AND  roleid=5 "; //se establece que solo se trabaje con los que tienen el rol de estudiante
$result = $DB->get_records_select($table,$select); //se ejecuta la sentencia establecida
 
      if(!empty($result)){//si se ejecuta la sentencia ingresa al condicional


       $length = strlen($output); //se recorre la salida que se mostrara al estudiante
        //Variables que almacenaran cadenas de texto
        $x1="";
        $x2="";
        $x3=true;
        $x4=true;
...
//Condicionales en donde inicia la comparación del texto plano
        if ($output[$i]==="<") 
            if ($output[$i+1]==="l")
                if ($output[$i+2]==="i")
                        $x3=false;
        ...
        if ($x4===false){
            $x3=true;
            $x4=true;
            $result = $DB->get_records("reac", array()); // obtiene todos los datos de la tabla reac

foreach($result as $record){     
     $variable=$record->id_module;
     $aux1=$record->id_user;
     $aux2=$USER->id;

       $table = 'reac';
            $select = "id_user = $USER->id "; 
            $res = $DB->get_records_select($table,$select); 
            //recorre y oculta los recursos y actividades que no se encuentran en la tabla
            if(strpos($aux1, $aux2)!==false){
                 if (strpos($x2, '<li class="activity '.$variable) !== false) {
             $x1.=$x2;
            }
            } 
}        
