...
// GNU General Public License for more details.logg
//
...
$select = "userid = $USER->id AND  roleid=5 "; //is put into the where clause
$result = $DB->get_records_select($table,$select); 
 
      if(!empty($result)){
             $table = 'historial_vark';
$select = "userid = $USER->id AND (respuestas='Visual' OR respuestas='Auditivo' OR respuestas='LeerEscribir' OR respuestas='Kinestesico')  "; //is put into the where clause
$result = $DB->get_records_select($table,$select);
  

          if (empty($result))
            redirect(new moodle_url('/my/preguntasVARK.php'));
           else {redirect($urltogo);} 

      }else{  unset($SESSION->wantsurl);
       redirect($urltogo);} 
...
if (isloggedin() and !isguestuser()) {
 ...
 //   $arrayaux=$user->profile;
   // if($arrayaux["Visual"]=="" and $arrayaux["Auditivo"]=="" and $arrayaux["LeerEscribir"]=="" and $arrayaux["Kinestesico"]=="" )
      // redirect(new moodle_url('/login/preguntasVARK.php'));
